class page {
  ...
  #_loadFields() {
    ...
    if (this.embed.fields.length < 25) {
      ...
      document.getElementById("addField").addEventListener("click", function () {
        User.proyect.actualPage.embed.addField({ name: "", value: "", inline: false })
        User.proyect.actualPage.previsualization.build()
      })
    } else {
      document.getElementById("fields").innerHTML = fieldsInnerHTML
    }
    document.querySelectorAll(".dfield").forEach(element => {
      element.addEventListener("click", function () {
        let fields = User.proyect.actualPage.embed.fields
        fields.splice(parseInt(element.id.split(/-/g)[1]), 1);
        User.proyect.actualPage.embed.setFields(fields)
        User.proyect.actualPage.previsualization.build()
      })
    })
    document.querySelectorAll(".cfield").forEach(element => {
      element.addEventListener("click", function () {
        User.proyect.actualPage.previsualization.save()
        User.proyect.actualPage.previsualization.build()
      })
    })
  }
  ...
  static updateColor() {
    if (!(document.getElementById("colorInput") && document.getElementById("colorLine"))) return
    document.getElementById("colorInput").value = User.proyect.actualPage.embed.color;
    document.getElementById("colorLine").style.backgroundColor = User.proyect.actualPage.embed.color;
  }
